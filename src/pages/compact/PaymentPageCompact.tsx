...
export const PaymentPageCompact = () => {
  ...
  const compactTotalInUsd = useMemo(() => formatUsd(convertTryToUsd(totals.grandTotal)), [totals.grandTotal]);
  const compactTotalInEur = useMemo(() => formatEur(convertTryToEur(totals.grandTotal)), [totals.grandTotal]);
    ...
    <div className="space-y-4 pb-28">
      <section className="sticky top-0 z-20 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur">
        ...
        <div className="mt-3 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
          <UserRound size={15} className="text-[#7A5B00]" />
          <span>Garson:</span>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2 text-center">
          <div>
            <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-slate-500">Adisyon</p>
            <p className="mt-1 text-xs font-semibold text-slate-900">{table.adisyonNo ?? '-'}</p>
          </div>
          <div>
            <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-slate-500">Tutar</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">{formatCurrency(totals.grandTotal)}</p>
          </div>
          <div>
            <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-slate-500">Kur</p>
            <p className="mt-1 text-[11px] font-semibold text-slate-900">{compactTotalInUsd}</p>
          </div>
        </div>
      </section>
        ...
        <div className="mb-2 flex items-center justify-between text-xs text-slate-500">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] font-medium text-slate-500">
              {compactTotalInUsd} • {compactTotalInEur}
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <AppButton variant="ghost" onClick={navigateBackToOrder}>
            Geri
          </AppButton>
          <AppButton
            onClick={() => {
            ...
            disabled={isSubmitting || paymentType === 'BOLUNMUS'}
          >
            {isSubmitting ? 'İşleniyor' : 'Direkt POS'}
          </AppButton>
        </div>
      </div>
    </div>
